...
class Trainer():
    def __init__(self,args):
        ...
        '''datafiles'''
        self.memory_file = f'data/{self.data_path}/factual.pickle'
        self.prompt_train_file = f'data/{self.data_path}/ltrain.csv' 
        self.prompt_dev_file = f'data/{self.data_path}/ldev.csv'
        self.prompt_test_file = f'data/{self.data_path}/ltest.csv'

        self.label_train_file = f'data/{self.data_path}/label_file/train_label.csv'
        self.label_dev_file = f'data/{self.data_path}/label_file/dev_label.csv'
        self.label_test_file = f'data/{self.data_path}/label_file/test_label.csv'
    ...
    def train(self,dataset,devdata=None,memory=None,template=None):
        # pretty print
        ...
        model = mnet.mT5Encoder(memory,template).to(self.device)
        if self.cycle:
            ...
            for ep in epoch:    
                ...
                with tqdm(range(total_batch),
                    ...
                    for i in tepoch:
                        ...
                        output = model(input_ids,attention_mask,expl=labels,sent_label=l,step_size=step_size,train=True)
                        loss = output.loss
    ...
    def evaluation(self,dataset):
        args = get_args()
        ckpt = self.ckpt_path
        mi = MemoryNet_Initializer(args.temp_size)
        memory,template = mi.loadData('dev')
        template = template.to(self.device)
        model = mnet.mT5Encoder(memory,template).to(self.device)
        ...
        with torch.no_grad():
            with tqdm(range(total_batch), unit=' samples',total = total_batch,mininterval=120,) as tepoch:
                for i in tepoch:
                    ...
                    output = model(input_ids,attention_mask,labels,sent_label=l,step_size=step_size,train=False)
    ...
    def test(self,dataset,memory=None,template=None):
        ckpt = self.ckpt_path
        template= template.to(self.device)
        model = mnet.mT5Encoder(memory,template).to(self.device)
...
def get_args():
    ...
    parser.add_argument('-temp_size',type=int,default=9)
    # continue training
...
def main():
    ...
    if train == 1:
        ...
        mi = MemoryNet_Initializer(size=args.temp_size)
        memory,template = mi.loadData('train')
        trainer.train(train_data,dev_data,memory=memory,template=template)
        ...
        memory,template = mi.loadData('test')
        trainer.test(test_data,memory=memory,template=template)
